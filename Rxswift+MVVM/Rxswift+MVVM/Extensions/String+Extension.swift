...
extension String {
    ...
    static func base64Encoding(_ plainString:String) -> String {
        if let plainData = plainString.data(using: String.Encoding.utf8) {
            let encodedString = plainData.base64EncodedString(options: Data.Base64EncodingOptions.init(rawValue: 0))
    ...
    static func base64Decoding(_ plainString:String) -> String {
        if let decodedData = Data.init(base64Encoded: plainString, options: Data.Base64DecodingOptions.init(rawValue: 0)) {
            if let decodedString = String.init(data: decodedData, encoding: String.Encoding.utf8) {
    ...
    func subStringFromTo(_ start:Int, _ end:Int)-> String {
        var substr = ""
        let start1 = self.index(self.startIndex, offsetBy: start)
        let end = self.index(start1, offsetBy: end - start)
    ...
    func subString(to index: Int) -> String {
        ...
        self.enumerateSubstrings(in: range, options: NSString.EnumerationOptions.byComposedCharacterSequences) { (substring, substringRange, enclosingRange, stop) -> () in
    ...
    func replacingCharacters(_ bounds:(startIndex:Int, length:Int),  with newElements:String) -> String {
        let s = self.index(self.startIndex, offsetBy: bounds.startIndex, limitedBy: self.endIndex)
        if nil != s {
            let e = self.index(s!, offsetBy: bounds.length, limitedBy: self.endIndex)
            if nil != e {
                return self.replacingCharacters(in: s!..<e!, with: newElements)
            }
        }
        return self
    }
    ...
    func filterAndLimitDigitsNum(_ limitDigitsNum:Int) -> String {
        ...
        var newString   = self.components(separatedBy: setToRemove).joined(separator: "")
